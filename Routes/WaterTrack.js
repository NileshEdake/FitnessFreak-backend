...
router.delete('/deletewaterentry', authTokenHandler, async (req, res) => {
    ...
    // Convert date from the request to ISO string
    const formattedDate = new Date(date).toISOString();

    // Filter the water entries to remove the one matching the formatted date
    user.water = user.water.filter(entry => new Date(entry.date).toISOString() !== formattedDate);

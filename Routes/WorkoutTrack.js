...
router.delete('/deleteworkoutentry', authTokenHandler, async (req, res) => {
    ...
    // Convert date from the request to ISO string (same as in the steps route)
    const formattedDate = new Date(date).toISOString();

    // Filter the workouts entries to remove the one matching the formatted date
    user.workouts = user.workouts.filter(entry => new Date(entry.date).toISOString() !== formattedDate);

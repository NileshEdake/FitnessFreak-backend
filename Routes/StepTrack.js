...
router.delete('/deletestepentry', authTokenHandler, async (req, res) => {
    ...
    // Filter the steps entries to remove the one matching the formatted date
    user.steps = user.steps.filter(entry => new Date(entry.date).toISOString() !== formattedDate);
